...
_INJECTION_PATTERNS: list[re.Pattern] = [
    ...
    re.compile(
        r"(new|updated|revised)\s+(system\s+)?(instructions?|prompt|rules?)\s*:"
        r"[^.\n]*",
        re.IGNORECASE,
    ),
    re.compile(
        r"(reveal|print|repeat|show|output)\s+(your|the)\s+"
        r"(system\s+prompt|instructions?|prompt)[^.\n]*",
        re.IGNORECASE,
    ),
    re.compile(
        r"do\s+not\s+(follow|obey|listen\s+to)\s+"
        r"(the\s+)?(previous|prior|system|original)[^.\n]*",
        re.IGNORECASE,
    ),
    re.compile(
        r"\[?\s*(system|assistant|developer)\s*\]?\s*:\s*you\s+(must|should|will)"
        r"[^.\n]*",
        re.IGNORECASE,
    ),
    re.compile(
