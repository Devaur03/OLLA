...
def test_all_ones_scores_one():
    b = svc.score(semantic=1, keyword=1, source_trust=1, freshness=1, feedback=1, density=1)
    assert b.final == 1.0


def test_all_zeros_scores_zero():
    b = svc.score(semantic=0, keyword=0, source_trust=0, freshness=0, feedback=0, density=0)
    assert b.final == 0.0
...
def test_semantic_dominates():
    ...
    high_sem = svc.score(
        semantic=1, keyword=0, source_trust=0, freshness=0, feedback=0, density=0
    ).final
    high_density = svc.score(
        semantic=0, keyword=0, source_trust=0, freshness=0, feedback=0, density=1
    ).final
...
def test_inputs_are_clamped():
    b = svc.score(
        semantic=5.0, keyword=-2.0, source_trust=0.5, freshness=0.5, feedback=0.5, density=0.5
    )
...
def test_breakdown_contributions_sum_to_final():
    b = svc.score(
        semantic=0.8, keyword=0.6, source_trust=0.7, freshness=0.5, feedback=0.4, density=0.3
    )
